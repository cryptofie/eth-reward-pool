...
module.exports = {
  ...
  networks: {
    ...
    ropsten: {
      url: "https://ropsten.infura.io/v3/21029a234342413683939eaec9e2c116", // <---- YOUR INFURA ID! (or it won't work)
...
task("stake", "Deposits the rewards into the contract")
  .addParam("amount", "Amount of ether to stake")
  .setAction(async (taskArgs, { ethers }) => {
    const provider = new ethers.providers.JsonRpcProvider(
    ...
    const tx = await rewardsContract.stake({
...
task("depositRewards", "Deposits the rewards into the contract")
  .addParam("amount", "Amount of ether to deposit")
  .setAction(async (taskArgs, { ethers }) => {
    const provider = new ethers.providers.JsonRpcProvider(
      config.networks[defaultNetwork].url
    );
    ...
    const tx = await rewardsContract.depositRewards({
      value: ethers.utils.parseEther(taskArgs.amount),
    });
    console.log(
      `Transaction sent on blockchain: https://ropsten.etherscan.io/tx/${tx.hash}`
